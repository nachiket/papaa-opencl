...
net_def = '../nets/pool_layer.prototxt'
...
# display all maps
for n in range(net.blobs['pool'].data.shape[1]):
    feat_map = exposure.rescale_intensity(net.blobs['pool'].data[0, n], out_range='float')
    cv2.imshow('pool_maps', feat_map)
    cv2.waitKey()
