...
int BuildDeviceKernel (char *xclbin) //added argv to param, mhho
{
	int err; 
...
int main(int argc, char **argv)
{
        ...
        for(i = 1; i < argc; i++)
	{
                ...
                } else if(!strcmp(argv[i], "-x")) { //added by mhho
                  xclbin = argv[++i];
                }
        }
	...
	if(!strcmp(mode, "sample"))
	{
		...
		if(BuildDeviceKernel(xclbin)) // added argv to param, mhho
		{
	...
	else if(!strcmp(mode, "test"))
	{
		...
		printf("Percentage Error: %f \n Number of Mispredicted images %d \n",((float)nMisPredict/(float)noTestImgs)*100,nMisPredict);
		destroyPGM(&input_pgm);
		fclose(fp);
		if(line)
			free(line);
