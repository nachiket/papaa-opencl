...
int main()
{
	   ...
	   readPGM(&input_pgm,"../pool/output3d0.pgm");
	   ipgm_img_width  = input_pgm.width;
	   ipgm_img_height = input_pgm.height;
	   opgm_img_width  = ipgm_img_width-CONV2_FILTER_WIDTH+1;
	   opgm_img_height = ipgm_img_height-CONV2_FILTER_HEIGHT+1;
	   ...
	   unsigned int mem_size_image = sizeof(DTYPE) * size_image;
           h_image  = (DTYPE*)malloc(mem_size_image * CONV2_NO_INPUTS);
	   
	   char filenameinput[50];
	   for(j=0;j<CONV2_NO_INPUTS;j++)
	   {
	       sprintf(filenameinput, "../pool/output3d%d.pgm",j);	
	   ...
	   unsigned int size_filter = CONV2_FILTER_WIDTH*CONV2_FILTER_HEIGHT;
	   ...
	   if (!kernel[0] || err != CL_SUCCESS)
	   {
 	   ...
 	   // Create the input and output arrays in device memory for our calculation
       	   d_image  = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR /*| CL_MEM_USE_MSMC_TI*/, mem_size_image*CONV2_NO_INPUTS, h_image, &err);
       	   d_filter = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR /*| CL_MEM_USE_MSMC_TI*/, mem_size_filter*CONV2_NO_INPUTS*CONV2_NO_OUTPUTS, h_filter, &err);
       	   d_output = clCreateBuffer(context, CL_MEM_WRITE_ONLY /*| CL_MEM_USE_MSMC_TI*/, mem_size_output*CONV2_NO_OUTPUTS, NULL, &err);
       	   d_bias   = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR , mem_size_bias*CONV2_NO_OUTPUTS, h_bias, &err);
	   ...
	   /*Retrieve result from device*/
           ...
           err = clEnqueueReadBuffer(commands, d_output, CL_TRUE, 0, mem_size_output*CONV2_NO_OUTPUTS, h_output, 0, NULL, NULL);
           if (err != CL_SUCCESS)
	   {
