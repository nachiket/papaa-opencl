...
void block_3d_conv(
	...
	int filter_start = out_map * K * K * no_inputs;
	// first KxK work items in each plane(z=0 --> NO_LOCAL_OUTPUT_MAPS) copy kernel coefficients
	const bool copy_ker = ((local_x < K) && (local_y < K));
	...
	// BLOCK_SIZE + K - 1 no of work items from plane 0 will copy 1 row each
	const bool copy_row = (local_z == 0 && (local_y * BLOCK_SIZE + local_x) < (BLOCK_SIZE + K - 1));
	int row_idx = local_y * BLOCK_SIZE + local_x;
	...
	for(uint imap = 0; imap < (no_inputs & 0x1FF); imap++) {
		...
		// copy block of input map to local buffer
		//
		// copy 1 row of the input map block with K-1 extra columns
		if(copy_row) {
			// copy extra row assigned to this work item
			#pragma unroll
		...
		// copy kernel for input map
		if(copy_ker) {
			map_ker[local_z][local_y * K + local_x] = p_weights[filter_start + (imap * K + local_y) * K + local_x];
		}
		...
		for(int kr = 0; kr < K; kr++) {
			#pragma unroll
			for(int kc = 0; kc < K; kc++) {
				sum += map_ker[local_z][kr * K + kc] * map_blk[local_y + kr][local_x + kc];
			}
		}	
		// wait for all work items to finish before overwriting local buffer.
		barrier(CLK_LOCAL_MEM_FENCE);
